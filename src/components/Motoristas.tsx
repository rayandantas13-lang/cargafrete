...
export default function Motoristas() {
  ...
  const salvar = () => {
    ...
    saveMotorista(editando ? { ...form, id: editando.id } : form);
    resetForm();
    carregar();
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">Cadastre os motoristas - salvo no navegador + Google Sheets</p>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div><label className="block text-sm font-medium text-slate-700 mb-1">Placa *</label><input value={form.placa} onChange={(e) => setForm({ ...form, placa: e.target.value.toUpperCase() })} className="w-full border border-slate-300 rounded-lg px-3 py-2" placeholder="ABC-1234" /></div>
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {motoristas.map((m) => (
                <tr key={m.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 font-mono">{m.placa}</td>
